module.exports = {
    ...
    'onBeforeExit': function(api){
        ...
        api.switchToIframe();
        result.pasties_position_css = api.evaluate(function collectCSS(){
            ...
            var wrapperElement = {'css': {}, clickHandler: element.onclick && element.onclick.toString()};
            ...
            if (firstChildElement !== null){
                firstChild.found = true;
                firstChild.clickHandler = firstChildElement.onclick && firstChildElement.onclick.toString();
                firstChild.name = firstChildElement.tagName;
                firstChild.html = firstChildElement.innerHTML;
                computedStyle = window.getComputedStyle(firstChildElement);
                KEYS.forEach(function(key){
                    firstChild.css[key] = computedStyle.getPropertyValue(key);
                });
            } else {
                firstChild.found = false;
                firstChild.html = element.innerHTML;
            }
