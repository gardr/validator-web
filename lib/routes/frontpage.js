...
function handler(request, reply) {
    ...
    if (site) {
        siteSettings.site = Hoek.clone(site);
        if(setup) {
            siteSettings.site.setup = Hoek.clone(setup);
            delete siteSettings.site.setup.viewport;
            delete siteSettings.site.setup.viewports;
            if(requestData.id && setup.viewports[requestData.id]) {
                siteSettings.site.setup.width = setup.viewports[requestData.id].width;
                siteSettings.site.setup.height = setup.viewports[requestData.id].height;
            }
        }
    }
    ...
    if (requestData.id && setup) {
        if (setup.viewports[requestData.id]) {
            viewport = setup.viewports[requestData.id];
        }
    }
