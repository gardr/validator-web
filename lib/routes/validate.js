...
internals.phantomWorker = function(data, next){
    log.info(data.id + ' processStamp', {processStamp: processStamp, id: data && data.id});
    getReport(data, function (err, harvest, report) {
        ...
        if (err) {
            ...
            log.error('An error occured. Code 002 (User input / Runnner Error):', {error: err, id: data.id});
        ...
        log.info('Formating done, saving result...', {'id': data.id});
...
internals.queue = function(id){
    if (!_queue){
        log.info('Creating new queue handler');
