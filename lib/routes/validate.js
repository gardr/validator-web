...
var ua = defaultOptions.userAgent;
...
function handler(req, reply) {
    ...
    var options = typeof payload.options === 'object' ? payload.options : {};
    ...
    var data = {
        ...
        // defaultOptions / pass-along
        'options': Hoek.merge(options, defaultOptions),
        'js': js,
        'html': html,
        'css': css,
        'zip': null,
    ...
    if (!data.viewport || typeof data.viewport === 'obejct' && Object.keys(data.viewport).length === 0){
        data.viewport = defaultOptions.viewport[data.target];
    }
    if (!data.viewport.width){
        data.viewport.width = defaultOptions.viewport[data.target].width;
    }
    if (!data.viewport.height){
        data.viewport.height = defaultOptions.viewport[data.target].height;
    }
...
function run(input) {
    ...
    getReport(input, function (err, harvest, report) {
        storage.get(input.id, function (err, data) {

            if (err || !data) {
                return storage.set(input.id, {
                    error: {
                        error: true,
                        message: 'Something went wrong. Errorcode 01'
                    }
                });
            }
