...
function handler(request, response) {
    ...
    if (!!hasCodePayload){
        ...
        // todo if not valid client url. do drop-off request and check for content-type.
        // drop-off request size can be checked against size-check-request
        if (data.url.indexOf('http') !== 0){
            // atm this doesnt stop next "loading"-view
            data.error = {
                error: true,
                message: 'Invalid url passed in'
            };
        } else {
            // lets fetch and be done
            done();
        }
