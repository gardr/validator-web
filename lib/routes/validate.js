...
function generateUrl(host, id) {
    return [host, '/user-entry.js?id=', id, '&timestamp=', Date.now()
    ].join('');
}
...
function getValidationData(id, payload, callback){
    ...
    var data  = {
        ...
        'formatIndex': payload.formatIndex || 0,
        'input': {
        ...
        'options': getSelectedConfig(payload.formatId, payload.formatIndex),
        'report': null,
        'harvest': null,
        'error': null
    };
 ...
 */
function getSelectedConfig(formatId, formatIndex) {
    var result,
        outputFormat;

    formats.some(function(format) {
        if (formatId == format.name) {
    ...
    if (result.formats && result.formats.length > 0){
        ...
        if (result.length <= formatIndex) {
            outputFormat = result.formats[formatIndex];
        } else {
            outputFormat = result.formats[0];
        }

        if (result.config) {
            // merge config wit default
            outputFormat.config = Hoek.merge(result.config, outputFormat.config);
        }

    } else {
        // error, missing formatlist
    }
...
var possibleHostHeaders = ["x-forwarded-host", "host"];
function getHost(headers){
    var result;
    possibleHostHeaders.some(function(key){
...
function handler(req, reply) {
    ...
    function runValidation(data){
        ...
        var host = getHost(req.raw.req.headers);
        if (host){
            data.host = host;
        }

        if ( hasCodePayload(data.input) ) {
            data.output.url  = generateUrl(data.host, id);
...
function requestInititalDataPayload(data, saveAndReply){
    ...
    function resultHandler(err, response, body) {
        ...
        if (contentType && contentType.indexOf('javascript') > -1) {
            data.base = getBase(data.output.url);
            data.output.js  = body.toString('utf8');
            data.output.url = generateUrl(data.host, data.id);
...
function run(_data) {
    ...
    getReport(_data, function (phantomError, harvest, report) {
         storage.get(_data.id, function (storageError, data) {
            if (storageError || !data) {
                log.error('Something went wrong. Code 001:', storageError);
            ...
            if (phantomError) {
                data.error = {
                    error: true,
                    err: phantomError,
                    message: 'An error occured. Code 002. '+phantomError.message
                };
                log.error('An error occured. Code 002 (User input / Runnner Error):', phantomError);
            ...
            if (hasPredefinedError || ((!phantomError || hasPredefinedError) && data && hasOutput)){
