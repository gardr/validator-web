...
internals.runUnzip =  function (uploadedZipFile, data, done) {
    ...
    unzipper.on('error', function (err) {
        ...
        data.error = {
            'message': 'An error occured with unzipping file.  Code 0XX. ' + err.message,
            'stack': err.stack + ''
        };
        cleanup();
        done();
    ...
    unzipper.on('extract', function (log) {
        ...
        if (!data.output.zip.indexFilePath) {
            var msg = 'Failed locating index.html file. Code 0XX';
            log.error(msg);
            data.error = {
                'message': msg
            };
        ...
        fs.readFile(data.output.zip.indexFilePath, function (err, fileBuffer) {
            if (err) {
                data.error = {
                    'message': 'Failed reading / locating index.html file. Code 0XX',
                    'err': err
                };
                return done();
            }
    ...
    unzipper.extract({
        ...
        filter: function (file) {
            /*
            {
              ...
              parent: '__MACOSX',
              filename: '._example',
              path: '__MACOSX/._example',
              type: 'File',
              mode: 384,
              compressionMethod: 8,
              modified: Wed Mar 19 2014 09:51:54 GMT+0100 (CET),
              crc32: 2575527935,
              compressedSize: 113,
...
module.exports = function (uploadedZipFile, data, done) {
    ...
    mkdirp(data.output.zip.path, function (err) {
        if (err) {
            var msg = 'Failed creating tmp zip output directory: ' + data.output.zip.path + '.  Code 0XX';
            log.error(msg);
            data.error = {
                'message': msg
            };
