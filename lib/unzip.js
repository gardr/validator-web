...
internals.runUnzip =  function (uploadedZipFile, data, done) {
    ...
    unzipper.on('extract', function (log) {
        ...
        fs.readFile(data.output.zip.indexFilePath, function (err, fileBuffer) {
            ...
            internals.validateIndex(data, html, done);
            ...
            data.output.html = parseAndResolve(base, html).full;
...
function filterErrors(a){
    return a.type === 'error' &&
        a.message !== 'Element “head” is missing a required instance of child element “title”.' &&
        a.message !== 'Almost standards mode doctype. Expected e.g. “<!DOCTYPE html>”.';
}

internals.validateIndex = function(data, html, done){
    internals.validateHTML(html, function(error, validationResult){
        if (error){
            console.log('w3c validation failed, reason: ', error);
            return done();
        }
