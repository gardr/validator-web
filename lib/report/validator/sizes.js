...
function outputSummary(summary, report, thresholdBytes, sizeThresholdBytes, options) {
    ...
    var restValue = metas.reduce(collectTotalMeta, 0);

    report.meta('Total'+(restValue > 0 ? ' rest value' : ' over threshold'), {
        restValue: restValue,
        success: restValue >= 0,
        error: restValue < 0
    });
...
function collectTotalMeta(total, value){
    var res=0;
    if (value.data){
        if (value.data.increase_threshold){
            res = value.data.increase_threshold;
        } else if (value.data.decrease){
            res = res - value.data.decrease;
        }
    }
    return total + res;
}
