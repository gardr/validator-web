    ...
    */
    ...
    if (summary){
        ...
        if (summary.typed.types.script){
            var scripts = Object.keys(summary.typed.types.script);
            if (scripts.length > 2){
                var trace = {trace: scripts.map(function(str){return {'sourceURL': str};})};
                report.warn('Please do not use ('+scripts.length+') more than maximum 2 external javascript files', trace);
            }
        }
