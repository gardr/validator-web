...
function validateRules(harvested, report, next) {
    ...
    var domData = harvested.gardrDom;
    if (domData) {
        validateWrapper(domData.wrapper, report);
        validateBannerCSS(domData.banner, harvested.gardrData, report);
        validateBannerDom(domData.banner, harvested.gardrData, harvested.windowOpened, report);
