...
function validate(harvested, report, next, options) {
    ...
    function analyze(content, sourceURL) {
        ...
        try {
            ast = esprima.parse(content.toString(), {'loc': true, 'tolerant': true});
        } catch(e){
            console.log(e);
            logger.info('Failed parsing javascript content', {type: typeof content, error: e});
    ...
    if (harvested.script.tags) {
        harvested.script.tags.forEach(analyze);
    }
